...
private:
    const int risk;
public:
    ForeignSalesWorker(const char* name, int money, double ratio, const int r) : SalesWorker(name, money, ratio), risk(r) {

    }
    int GetLisskPay() const {
        return (int)(SalesWorker::GetPay() * (risk / 100.0));
    }

    int GetPay() const
    {
        return SalesWorker::GetPay() + GetLisskPay();
    }
    ...
    void ShowSalaryInfo() const
    {
        ...
        cout << "risk pay: " << GetLisskPay() << endl;
